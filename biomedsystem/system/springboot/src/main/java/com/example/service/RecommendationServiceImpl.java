...
public class RecommendationServiceImpl implements RecommendationService {
    ...
    public Map<String, Object> assistantAsk(String question, Long userId, String role) {
        ...
        if (normalized.contains("未入库")) {
            ...
            result.put("answer", "当前未入库样本共 " + target.size() + " 条。下面列出最近的部分样本。给样本绑定 storageId 后，这个预警就会消失。");
        ...
        if (normalized.contains("待处理")) {
            ...
            result.put("answer", "当前待处理样本共 " + target.size() + " 条。下面列出最近的部分待处理样本。\n如果某些样本同时没有 storageId，它们也会在首页显示为未入库预警。");
            result.put("relatedSamples", buildRelatedSamples(target, 10));
            return result;
        }

        if (normalized.contains("最近7天") || normalized.contains("近7天")) {
            List<BiomedSample> target = samples.stream()
                    .filter(s -> s.getCollectTime() != null && s.getCollectTime().isAfter(now.minusDays(7)))
                    .toList();
            result.put("answer", "最近7天新增/采集的样本共 " + target.size() + " 条。下面列出最近的部分样本。\n如果你想继续问，可以输入：最近7天未入库样本有哪些？");
            result.put("relatedSamples", buildRelatedSamples(target, 10));
            return result;
        }

        if (normalized.contains("类型") && (normalized.contains("最多") || normalized.contains("哪个"))) {
            Map<Long, Long> counter = samples.stream()
                    .filter(s -> s.getSampleTypeId() != null)
                    .collect(Collectors.groupingBy(BiomedSample::getSampleTypeId, Collectors.counting()));
            if (counter.isEmpty()) {
        ...
        if (normalized.contains("总数") || normalized.contains("多少样本") || normalized.equals("样本有多少")) {
            result.put("answer", "当前样本总量为 " + samples.size() + " 条。\n你还可以继续追问：未入库样本有哪些？异常样本有多少？哪个样本类型最多？");
            result.put("relatedSamples", Collections.emptyList());
            return result;
        }
        ...
        if (bioToken != null) {
            List<BiomedSample> target = samples.stream()
                    .filter(s -> s.getSampleNo() != null && s.getSampleNo().contains(bioToken))
                    .toList();
        ...
        List<BiomedSample> fuzzy = samples.stream()
                .filter(s -> containsIgnoreCase(s.getSampleName(), normalized)
                        || containsIgnoreCase(s.getSource(), normalized)
                        || containsIgnoreCase(s.getSampleNo(), normalized))
                .toList();
    ...
    private List<Map<String, Object>> buildRelatedSamples(List<BiomedSample> list, int limit) {
        return list.stream().limit(limit).map(sample -> {
            ...
            item.put("sampleName", sample.getSampleName());
